...
FRAMEWORK_ROOT = platform.get_package_dir("A21B")
...
def configure_crystal_frequency(cpp_defines, board, variant):
    if "CRYSTAL_FREQ" not in env.Flatten(cpp_defines):
        freq = 8
        if board.startswith("blackpill_f401") or variant == "generic_f407v_mini":
            freq = 25
        env.Append(CPPDEFINES=[("CRYSTAL_FREQ", freq)])
...
def configure_application_offset(board_name, upload_protocol):
    ...
    env.Append(CPPDEFINES=[("USER_ADDR_ROM", addr)])
...
env.Append(
    ...
    LINKFLAGS=[
        ...
        "-Wl,--wrap=atexit,--wrap=__cxa_atexit,--wrap=exit"
    ],
...
configure_crystal_frequency(cpp_defines, board_name, variant)
